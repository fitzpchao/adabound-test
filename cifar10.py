...
def create_optimizer(args, model_params):
    if args.optim == 'sgd':
        return optim.SGD(model_params, 0.1, momentum=0.9,
                         weight_decay=args.weight_decay)
    elif args.optim == 'adagrad':
        return optim.Adagrad(model_params, args.lr, weight_decay=args.weight_decay)
    elif args.optim == 'adam':
        return optim.Adam(model_params, 0.001, betas=(0.99, 0.999),
                          weight_decay=args.weight_decay)
    elif args.optim == 'amsgrad':
        return optim.Adam(model_params, args.lr, betas=(args.beta1, args.beta2),
                          weight_decay=args.weight_decay, amsgrad=True)
    elif args.optim == 'adabound':
        return AdaBound(model_params, args.lr, betas=(0.9, 0.999),
...
def main():
    ...
    optimnames = ['sgd', 'adam', 'adabound']
    for optimname in optimnames:
        ckpt_name = get_ckpt_name(model=args.model, optimizer=optimname, lr=args.lr,
        ...
        for epoch in range(start_epoch + 1, 200):
            ...
            if not os.path.isdir('curve_densenet'):
                os.mkdir('curve_densenet')
            torch.save({'train_acc': train_accuracies, 'test_acc': test_accuracies},
                       os.path.join('curve_densenet', ckpt_name))
        end = time.time()
        times = end - start
        print(times)
        times = np.array([times], np.float32)
        np.save(os.path.join('times', ckpt_name + '_times.npy'), times)
